...
### $\mu$Fork
$\mu$Fork improves the compatibility of single-address-space OSes (SASOSes), allowing them to efficiently and securely support multi-process applications.
...
- Intra-Address-Space Isolation: We utilize CHERI’s hardware capabilities to enforce strict isolation between "$\mu$processes" and the kernel, preventing unauthorized access despite sharing a single address space.

To maintain high performance, we redesigned the traditional Copy-on-Write (CoW) mechanism. In a SASOS, standard CoW is insufficient because a child reading a pointer would still see the parent's address. We developed Copy-on-Pointer-Access (CoPA), an optimization that allows memory sharing until a write occurs or until the child loads a pointer10.This ensures pointers are relocated lazily only when necessary, significantly reducing fork latency compared to full copying.

We show that $\mu$Fork is able to outperform traditional monolithic OSes on metrics such as fork latency by an order of magnitude. Find the paper [here](https://jkressel.github.io/files/ufork-sosp2025.pdf).
